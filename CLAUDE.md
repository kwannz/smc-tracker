...
- **★ 信号可信度地图（#150-170 样本外实证审计，单一真相源，禁止再建在已证伪前提上）**：
  - **有 edge（系统真本事=测幅度/极端/形态/逼空，非测温和方向）**：
    · 波动**水平预测**(EWMA→未来 h-bar 平均已实现波动) 有**扎实技巧**:corr 0.30(1bar)→0.42(5bar)→0.45(10bar)随视野升(#178)=真实幅度 edge;
      逐 bar |收益|记忆快衰减(#149 ARCH 自相关 lag-1≈0.28→lag-10≈0.05,null≈0);EWMA 比朴素 rv 增益小偏长视野(#155-159)。
