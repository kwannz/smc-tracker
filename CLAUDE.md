...
- **★ 信号可信度地图（#150-194 样本外实证审计，单一真相源，禁止再建在已证伪前提上）**：
  - **🏛️ 基石(系统根本命题,#185 首次实证)：聪明钱技巧真实持续，非运气**：地址早半→晚半(fills 按时间切非重叠)
    ...
    **"发现庄"(识别技巧持续的庄)有据**;但"**跟庄=复制入场即盈利**"那半 #192 已证伪(见下),故命题精确为"识别庄准、复制入场不盈利"。
    ⚠陷阱:**原始 PnL 持续 −0.295**(被账户规模+均值回归污染),必须用胜率/效率度量(#149纪律)。
  ...
  - **有 edge（系统真本事=测幅度/极端/形态/逼空，非测温和方向）**：
    ...
    · 谐波 setup 经验盈利 +0.5R/笔(#165，confidence 真校准:0.9桶 +1.5R)。
      **#206 大样本回测确认(15币×2000bar 1H,826笔真实)**:合计 55.8%胜/+0.67R/2.53盈亏比(15币全正期望);
      **置信门控强验证**:min_conf≥0.75 → **78.1%胜/+1.34R/7.13盈亏比/回撤14R→6R**(更少交易但四维全优)⇒confidence 真分离会赢/碰运气,#169 推送门控有据。谐波交易机器人正期望可行(gross,scripts/run_harmonic_strategy.py)。
      **#208 多因子汇合裁决(15币×2000bar)——最优=谐波+S/R+置信≥0.75(78.8%胜/+1.36R/7.43盈亏比/6R回撤)**:Fib 谐波已含;**S/R 确认真有用**(D反转位∩经典S/R,55.9→57.6%胜、回撤降);
      置信门控碾压;**SFG 必须排除**(单独加掉到49%胜,印证#205 SFG喂≈随机)、"全汇合"反退化(65%,SFG拖累+过度过滤)。⇒**因子越多≠越好,砍有害(SFG)留互证(S/R+置信)**。MTF 多周期门控待接。CLI:backtest --harmonic --require-sr --min-conf 0.75。
    · ~~逼空背离 +0.83pp(#170)~~ **#193 降级 UNVERIFIED**:与 #186/#187 **同类**(小 coin 样本前瞻收益,n=17 极小),该类已两次币内配对证伪翻转
      (#186 +0.46%↔−0.53%、#187 +7.1%↔−6%);n=17 无法验证⇒**逼空 edge 未确立、勿加码**。**方向类至此无确立 edge**(与"方向≈鞅"彻底一致)。拆 kind(#176)保留仅为生产持续审判(以 efficacy 实盘为准)。
    · **🛡️ 方法论免疫(#193 系统性应用 #192 教训)**:凡"小 coin 样本上的聚合前瞻收益"测得的 edge 都对**币种选择极敏感、会随覆盖符号翻转**,
      必须**币内配对 + 改善覆盖**复核(#186/#187/#170 皆此类,已证伪/降级)。**稳健 edge(非此类,故站得住)**:#185 币内胜率持续(per-address split-half,size-independent)、
      vol 预测(币内自身时序,#180 跨周期泛化)、pump lift(25万点 OOS 条件概率非币子集均值)、谐波 R(大样本 per-setup)。新 edge 上线前先问"是否该类、是否币内配对过"。
...
- **抓庄核心**：发现庄(排行榜)→ 实时监控成交/持仓 → 跟庄/换仓/共识/可疑地址/关联(庄家集团) → 信号。
  **实证定位(#185-193 校准,北极星)**：系统真实价值=**①波动幅度预测(GARCH,真 edge)+ ②聪明钱注意力探测**("庄技巧持续#185,看它在哪活跃")；
  **非"照庄入场复制盈利"**(#192 证伪)。开发优先强化 ①② 与 pump/谐波(真 edge),勿再朝"优化跟庄复制盈利"用力(已证伪前提)。
