...
- **★ 信号可信度地图（#150-183 样本外实证审计，单一真相源，禁止再建在已证伪前提上）**：
  - **🏛️ 基石(系统根本命题,#185 首次实证)：聪明钱技巧真实持续，非运气**：地址早半→晚半(fills 按时间切非重叠)
    ...
    **#187 第二基石(共识/address_correlation)——非单调,"庄越多越强"证伪**:同币同向 4h 窗内庄数分桶前瞻 alpha(扣漂移):
    单庄 +1.8%(24h)< **双庄 +7.1%(峰值)** > 多庄≥3 +2.0%(塌回单庄,拥挤反转signature,同 funding 拥挤~0)。**共识(≥2)有信息但倒U,非线性放大**。
    ⚠设计张力(待更多数据再改,证据尚薄 n~540/桶、幅度或噪声膨胀):`WhaleConsensus` min_consensus=3 会**滤掉最强双庄信号**、score∝庄数给≥3拥挤更高分——与实证反向;勿据≥3强共识加码。脚本 scripts/audit_consensus_strength.py。
