...
- **★ 信号可信度地图（#150-170 样本外实证审计，单一真相源，禁止再建在已证伪前提上）**：
  - **有 edge（系统真本事=测幅度/极端/形态/逼空，非测温和方向）**：
    · 波动**水平预测**有**扎实技巧**,主前瞻量=**GARCH(1,1)**(均值回归,#179 真实25币15m各视野胜 EWMA +0.07:corr 0.38/0.49/0.53@1/5/10bar);
      EWMA(0.30/0.42/0.45,#178)是其 α+β=1 退化特例(假设随机游走漏掉回归);逐 bar |收益|记忆快衰减(#149 ARCH 自相关 lag-1≈0.28→lag-10≈0.05,null≈0)。
