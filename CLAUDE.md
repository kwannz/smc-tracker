...
- **★ 信号可信度地图（#150-183 样本外实证审计，单一真相源，禁止再建在已证伪前提上）**：
  - **🏛️ 基石(系统根本命题,#185 首次实证)：聪明钱技巧真实持续，非运气**：地址早半→晚半(fills 按时间切非重叠)
    ...
    **#186 跟庄可操作性(续 #185 第二道坎)**:庄 Open 入场**领先价格**(HL 15m,扣币种漂移=alpha 非 beta,5111 笔):0延迟 4h **+0.46%**/24h **+0.97%**。
    **视野分化(产品边界)**:短视野(≤4h)edge **对延迟敏感**(1h延迟→4h 归零 −0.09%)⇒需低延迟检测(WS实时流式被证值得);
    长视野(24h)**扛延迟**(1h延迟仍 +0.81%)⇒慢跟(轮询)宜瞄 24h 长持。脚本 scripts/audit_copytrade_latency.py。局限:gross(无手续费/滑点)、验入场方向非全 round-trip。
    **#187→#188 第二基石(共识/address_correlation)——UNESTABLISHED(证据不可靠,不下结论)**:
    #187 初测似"双庄 alpha 峰值(+7.1%@24h)、≥3 反转"非单调,但 #188 加中位数+bootstrap+扩样本**严格复核推翻**:
    双庄 24h alpha **+7.1%↔−6.0% 跨运行符号翻转**——核心限制:**共识事件集中少数币,前瞻 alpha 由"哪些币 candle 加载"主导**(HL 限流致两次加载不同币子集),单次 bootstrap CI 看似显著但跨运行不稳定碾压之。
    诚实结论:**当前数据/限流下无法确立共识是否放大信号(既未证实也未证伪)**;`WhaleConsensus` min_consensus=3/score∝庄数 设计**不据此改**(#187 "设计反向" 宣称撤回)。需更大无限流样本+per-coin 控制才能定论。脚本 scripts/audit_consensus_strength.py。
    🎓教训:单次样本内 bootstrap CI 可"显著"却跨样本翻转——主导不确定性(币种选择)不在重采样里;幸亏 #187 守"薄证据不改算法",翻盘零伤害。
