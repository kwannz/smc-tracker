...
class WhaleConsensus:
    ...
    ⚠**#187→#188→#191 共识放大≈0(已了结):多庄不比单庄预测更大移动**。#187 似非单调(双庄峰值)、#188 复核翻转疑伪影,
    #191 用 #189 retry 改善覆盖(狂摆消失=证实覆盖伪影)+ **币内配对**(消除币种选择混淆)定论:币内 cons−solo 24h 中位 −0.47pp、
    6/12币为正(≈掷硬币)⇒共识放大≈0,归"≈0噪声"。**精确**:非"共识无用"——每庄背后是技巧持续的庄(#185),
    只是入场点不可复制盈利(#192 推翻 #186)、共识也不额外放大⇒信号价值=**"聪明钱活跃"语境**,非跟单/加码盈利。
    故 score∝n_agree 当"更强"**无据**(勿按庄数加权)、min_consensus=3 应理解为**去噪阈值**(够多庄才发以滤偶发)非"越多越强"。
