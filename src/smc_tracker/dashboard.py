...
def render_monitored_page() -> str:
    """监控清单管理迷你页（自包含 HTML，无 CDN）。

    独立于主页 _HTML_TEMPLATE（后者用 {{/}} 转义，注入 JS 易腐蚀括号）：此页用普通
    字符串，花括号即字面量，安全。前端 fetch /api/monitored 增删查，5s 无依赖。
    """
...
async def serve(db_path: str, host: str = "127.0.0.1", port: int = 8787) -> None:
    ...
    async def handle_monitored_page(request: aiohttp.web.Request) -> aiohttp.web.Response:
        """GET /monitored — 监控清单管理迷你页（自包含 HTML，独立于主页模板）。"""
