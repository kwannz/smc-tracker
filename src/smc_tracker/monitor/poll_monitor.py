...
class PollMonitor:
    ...
    def _digest(self, whales, positions, changes, cons, confl, panel, flow, divs,
        ...
        # #192 撤回 #190 标注:#186 入场领先 edge 经币内配对证伪(coin-selection 伪影,扣成本不赚);
        # 庄技巧持续(#185)但盈利非来自可复制入场timing→建仓信号作"聪明钱活跃"语境,非跟单盈利保证。
        lines.append(f"\n🐋 庄群近窗主动建仓 {len(builds)} 条（聪明钱活跃语境;照入场跟单未验证盈利#192）：")
