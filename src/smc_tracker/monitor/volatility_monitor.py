...
设计（CLAUDE.md：低延迟 + 模块化扁平 + 极简；用户#：不做共振，每周期各显指标 + PDArray）：
  ...
  - **诚实标注（信号性质，CLAUDE.md §二）**：本模块全部指标（rv/velocity/accel/vol_ratio/regime/pdarray）
    ...
    **诚实量(须分两个不同对象,#177→#178 修矫枉过正,#179 升级模型)**：
    ① 波动**水平预测**有扎实技巧,**主前瞻量=GARCH(1,1)**(均值回归,EWMA 是其 α+β=1 退化特例)：
       15m corr 0.38(1bar)→0.49(5bar)→0.53(10bar);**胜 EWMA 周期依赖(#180):15m+0.078 最强、4H/1D+0.02、1H≈中性**(短周期获益最大)。
...
# GARCH(1,1) 固定参数(免拟合,可进热路径):#179 实测 15m 胜 EWMA +0.078 corr;
# 固定 α0.10/β0.85(α+β=0.95)稳健、网格最优 α0.14/β0.75 同向。Bollerslev 1986。
...
def garch_vol(c: Any, alpha: float = _GARCH_A, beta: float = _GARCH_B) -> float:
    ...
    波动随机游走、永远预测"当前=未来")更准。**优势周期依赖(#180 跨周期泛化,类 #159)**：
    **15m 最强 +0.078**(corr 0.38/0.49/0.53 vs EWMA 0.30/0.42/0.45)、4H/1D 温和 +0.02、1H≈中性 +0.002(长视野微负,采样噪声内)——
