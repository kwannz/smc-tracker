...
设计（CLAUDE.md：低延迟 + 模块化扁平 + 极简；用户#：不做共振，每周期各显指标 + PDArray）：
  ...
  - **诚实标注（信号性质，CLAUDE.md §二）**：本模块全部指标（rv/velocity/accel/vol_ratio/regime/pdarray）
    ...
    **诚实量(#149/#177 ARCH 标准 |logret| 自相关,无重叠)：lag-1≈0.28、lag-5≈0.12、lag-10≈0.05(快速衰减但 null≈0=真实)**——
    故近 1~数 bar 波动水平**弱可测**(GARCH 同理:波动可测、收益不可测),但**非"90%续"**;而**方向**(velocity/PD)短期反转不可赌(#150-152)。
    系统定位=测波动水平短记忆,非择时方向、非"高持续"。脚本 scripts/audit_expansion_persistence.py 可复现。
...
class VolatilityMonitor:
    ...
    def render(self, rows: list[dict], now_ms: int = 0, top: int = 8) -> str:
        ...
        if hl["expansion"]:
            ...
            lines.append("🔶放量(扩张·已放大;波动弱短记忆≠90%续、方向不定): " + " ".join(
