...
设计（CLAUDE.md：低延迟 + 模块化扁平 + 极简；用户#：不做共振，每周期各显指标 + PDArray）：
  ...
  - **诚实标注（信号性质，CLAUDE.md §二）**：本模块全部指标（rv/velocity/accel/vol_ratio/regime/pdarray）
    ...
    **前瞻价值的不对称(#153 实测,本系统核心结论)——信幅度别信方向**：波动**水平**高度持续可测
    (扩张后10bar内仍扩张90%、lift7.6×；corr(当前rv,10bar后rv)=0.73)，故实时追踪 **rv/HVP/regime 波动水平有真实前瞻价值**
    (GARCH 同理:波动可测、收益不可测)；而**方向**(velocity/PD)短期反转不可赌(#150-152)。系统定位=测波动水平,非择时方向。
