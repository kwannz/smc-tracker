...
def smart_money_score(
    ...
    *,
    ...
    ① 跨窗一致性(周&月&全期同时为正)=持续 edge，过滤一次性运气；
       **#185 实证背书**:地址早→晚半技巧(胜率/效率)持续 corr ~0.51(null ~0)——"技巧持续非运气"前提获样本外验证,
       此一致性判别器的设计有据(scripts/audit_smart_money_persistence.py);注:须用 size-independent 度量,原始 PnL 持续为负(规模污染)。
