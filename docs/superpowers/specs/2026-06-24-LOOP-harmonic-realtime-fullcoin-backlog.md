...
- 2026-06-25 loop#22(Opus 视觉验证 + 排版修复):**浏览器实际打开两页截图验证排版**(回应用户「网页排版
  细节没问题再部署」)。谐波页:7 周期 tab/分页(1/3·124币)/搜索/过滤/XABCD+PRZ 图/多周期 S/R 7 周期表/
  交易计划 setup/KNN 诚实标注(≈随机)全部正常专业。HL 页:聪明钱地址排行/庄家集团(跨币协同)/净主动流向
  瀑布/鲸鱼动作流正常;H4 tab 双向切换流畅。**抓到 1 真排版 bug**:右栏鲸鱼动作 badge(hl-event-icon 固定
  尺寸)内长 label「庄#4(PnL$140M)」撑破溢出栏外 → badge 只取 '(' 前短标识(split),完整 PnL 留副标题
  (8aac213,164 dashboard 测试绿)。**洞察**:node --check 保 JS 语法但抓不到视觉溢出,真浏览器截图才发现,
  「排版细节」须眼见为实。下一步:回填完成度核验 → D2/E2 → 部署门禁(须批准)。
