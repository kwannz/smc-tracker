...
- [ ] **交易所资金流热钱包低估 / Bitget BTC 地址**（诚实保留）：`exchange_flow.py` BTC 分页对极端高频热钱包
      （24h>150 笔）低估系**公开数据固有局限**；`exchange_wallets.yaml:21` Bitget BTC `[]` —— **无已确认的公开
      Bitget BTC 冷/热钱包地址可实证**（第一性原理不编造地址）。待获官方/链上标注确认地址后补全闭合。
...
- [ ] **真实部署 alpha 验证**（环境约束，不可沙箱闭合）：#95 诚实发现本沙箱外部 API 实时性不可信（缓存快照），
      系统逻辑正确但 **alpha 须真实部署长跑验证**；KNN≈随机已诚实标注（仅辅助）。生产 `43.224.34.216` 已长期运行，
      真实数据累积中——此项**本质是持续验证任务，非一次性代码闭合**，保持开放追踪。
