...
- [~] **真实部署 alpha 验证（#97 首次生产复盘完成，持续追踪）**：生产 `43.224.34.216` 真实累积数据**首次 alpha 复盘**
      ...
      此项本质持续验证（样本累积中），保持开放。
- [ ] **🆕 背离/暴涨信号 0% 命中复查（#97 alpha 复盘暴露）**：生产 111 评估中背离 0/46、暴涨 0/12 命中 ——
      大样本 0% 强烈提示**信号方向反向**（或评估口径/水平线问题）。需复查 `_on_divergence`/`pump_radar` 的 direction
      映射与 `review` 评估对齐；若确为反向，反手即正期望（contrarian），是高价值修正。
