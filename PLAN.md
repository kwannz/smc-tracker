...
## 迭代日志
- 2026-06-27 #208: **能实现:谐波×Fib×S/R×置信多因子汇合(用户问)——数据裁决最优=谐波+S/R+置信≥0.75,SFG排除**（ralph loop 续;Opus build+validate）。
  用户问"谐波结合多周期+SFG+斐波那契+支撑压力位能否实现"——拼图全在(Fib谐波已含/levels.py S/R/mtf_confluence MTF/sfg_consensus)。本轮接 S/R 确认(`require_sr`:谐波入场近同向支撑/压力,_near_sr近窗零前视)+CLI --require-sr。
  **15币×2000bar 真实回测裁决组合**:谐波基线55.9%胜/+0.68R;**+S/R 57.6%/+0.73R(真有用,D反转∩经典S/R互证)**;+SFG **49.2%(拖累,印证#205)**;+置信≥0.75 78.1%/+1.34R;
  **🏆 +S/R+置信 78.8%胜/+1.36R/7.43盈亏比/6R回撤(最优)**;全汇合(含SFG)65.4%(退化:SFG拖累+过度过滤剩280笔)。
  **结论:能实现且已知最优配方——砍有害(SFG)、留互证(S/R+置信)、Fib已含;因子越多≠越好。MTF多周期门控待接(下轮)。** TDD:require_sr过滤≤基线+_near_sr。全量2491 passed,103行≤800。
  教训:多因子组合最该用数据而非直觉决定——用户设想的全家桶里,Fib/S/R是好搭档、置信核心、SFG负担;诚实=砍掉拖后腿的因子而非堆砌。

- 2026-06-27 #207: **大样本真实数据谐波策略回测:826笔正期望(56%胜/+0.67R),置信门控强验证(≥0.75→78%胜/+1.34R)**（ralph loop 续;用户"跑backtest看策略";Opus 验证）。
  #206 回测高效后,scripts/run_harmonic_strategy.py 对 15币×2000bar 真实 1H 跑谐波策略(无SFG,#205已验不帮忙):
  ① **15币全部正期望**,合计 826笔 55.8%胜/+0.67R期望/2.53盈亏比/+557R(NEAR+1.19R/ADA+0.94R佳,DOGE+0.28R最弱仍正)——谐波bot稳健正期望非小样本侥幸;
  ② **置信分层钉死 #165 校准**:min_conf≥0.75 → 交易826→543但 **胜率56%→78%、期望+0.67→+1.34R、盈亏比2.53→7.13、回撤14R→6R**——四维全优,confidence 真分离会赢/碰运气,#169 推送门控(≥0.75)有据。
  耗时83.7s(15币×2000bar×4配置,多为采集)。诚实:gross 未扣费/滑点(谐波以R计、止损含R内,0.1%成本相对R小,净仍强正)、近83天1H样本。写可信度地图。纯脚本无src改动。
  教训:回测先做高效(#204-206)才能大样本验证;谐波bot方向(#201-207)收获实证回报——能跑/够快/真赚;置信门控不是装饰,大样本证它真值钱。
